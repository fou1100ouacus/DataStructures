...
void Diffrence2list(node*head1,node *head2) ;

void moveZeros to_front (node *head) ;
...
int HLeader(node*head)
{
	...
	while(run1!=NULL){
		node*run2;
		run2=head;
	while(run2!=run1&&run2->data<run1->data){
		run2=run2->next;
	}
...
while (temp!=NULL) {
    ...
    if (temp->data == 0 ) {
node * current = temp;
temp=temp->next;/// counter loop
prev->next=temp;
///InSertionFirst(head,current->data) ;
current->next=head;
head=current;
    }
    else {
